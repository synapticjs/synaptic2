...
`.toLowerCase().replace(/([^ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz\-\(\)\.\!‘’\?,\n ])/g, '');
...
function fillChar(letter) {
  let a = new Array(letters.length).fill(0);
  a[letters.indexOf(letter)] = 1;
  return a;
}


ts.push({ input: fillChar('\n'), output: fillChar(text[0]) });

for (let i = 1; i < text.length; i++) {
  let actual = text[i - 1];
  let expected = text[i];

  ts.push({ input: fillChar(actual), output: fillChar(expected) });
}
